...
export default function Onboarding() {
  ...
  const handleLogin = async (provider: "github" | "google", router:ReturnType<typeof useRouter>, setStep:React.Dispatch<React.SetStateAction<number>>) => {
    try {
      ...
      if (data.exists) {
        toast("User exists, redirecting to dashboard...");
  ...
  const handleFinish = handleSubmit(async (data) => {
    try {
      const response = await fetch("http://localhost:4444/createuser/", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      toast("An error occurred during onboarding.");
    }
  });
        ...
        {step === 2 && (
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-1 gap-2">
              <Button
                variant={userData.type === "creator/collaborator" ? "default" : "outline"}
